...
// .wrangler/tmp/bundle-YG7cK3/checked-fetch.js
...
// .wrangler/tmp/bundle-YG7cK3/strip-cf-connecting-ip-header.js
...
var AppError = class extends Error {
  ...
  static badRequest(message2) {
    return new AppError(400, message2);
  }
  static unauthorized(message2 = "Unauthorized") {
    return new AppError(401, message2);
  }
  static forbidden(message2 = "Forbidden") {
    return new AppError(403, message2);
  }
  static notFound(message2 = "Not found") {
    return new AppError(404, message2);
  }
  static conflict(message2) {
    return new AppError(409, message2);
  }
};
...
var registerSchema = external_exports.object({
  ...
  role: external_exports.enum(["player", "organizer"]).default("player")
});
...
var createTeamSchema = external_exports.object({
  ...
  homeVenue: external_exports.string().optional()
});
...
var invitePlayerSchema = external_exports.object({
  email: external_exports.string().email()
});

// src/models/base.ts
var baseEntitySchema = external_exports.object({
...
var competitionStatus = external_exports.enum([
  "draft",
  "upcoming",
  "active",
  "completed"
...
var gameDefinitionSchema = external_exports.object({
  ...
  type: external_exports.enum(["game", "break"])
});
var scheduleConfigSchema = external_exports.object({
  venueType: external_exports.enum(["central", "team_venues"]),
  centralVenue: external_exports.string().optional(),
  gamesPerOpponent: external_exports.number().min(1).max(2).default(1),
  schedulingType: external_exports.enum(["weekly_rounds", "fixed_matchdays"]),
  weekdays: external_exports.array(external_exports.number().min(0).max(6)).optional(),
  fixedDates: external_exports.array(external_exports.string()).optional()
});
var competitionSchema = baseEntitySchema.extend({
  name: external_exports.string(),
  organizerId: external_exports.string(),
  gameType: external_exports.string().optional(),
  format: external_exports.enum(["teams"]).default("teams"),
  tournamentType: external_exports.enum(["round_robin"]).default("round_robin"),
  startDate: external_exports.string().optional(),
  prize: external_exports.number().optional(),
  status: competitionStatus.default("draft"),
  teamSizeMin: external_exports.number().min(1).default(2),
  teamSizeMax: external_exports.number().min(1).default(5),
  gameStructure: external_exports.array(gameDefinitionSchema).optional(),
  scheduleConfig: scheduleConfigSchema.optional()
});

// src/schemas/competition.schema.ts
var createCompetitionSchema = external_exports.object({
  name: external_exports.string().min(1),
...
var updateCompetitionSchema = external_exports.object({
  name: external_exports.string().min(1).optional(),
  description: external_exports.string().optional(),
  gameType: external_exports.string().optional(),
  startDate: external_exports.string().optional(),
  ...
  city: external_exports.string().optional(),
  country: external_exports.string().optional(),
  teamSizeMin: external_exports.number().min(1).optional(),
  teamSizeMax: external_exports.number().min(1).optional(),
  gameStructure: external_exports.array(gameDefinitionSchema).optional(),
  scheduleConfig: scheduleConfigSchema.optional()
});
var applicationActionSchema = external_exports.object({
  action: external_exports.enum(["accept", "reject"])
});

// src/models/match.ts
var matchStatus = external_exports.enum(["scheduled", "in_progress", "pending_review", "completed"]);
var gameResultSchema = external_exports.object({
  gameOrder: external_exports.number(),
  homePlayerName: external_exports.string().optional(),
  awayPlayerName: external_exports.string().optional(),
  homeScore: external_exports.number().default(0),
  awayScore: external_exports.number().default(0)
});
var matchSchema = baseEntitySchema.extend({
  competitionId: external_exports.string(),
  round: external_exports.number(),
  matchday: external_exports.number(),
  homeTeamId: external_exports.string(),
  awayTeamId: external_exports.string(),
  homeTeamName: external_exports.string(),
  awayTeamName: external_exports.string(),
  scheduledDate: external_exports.string().optional(),
  venue: external_exports.string().optional(),
  status: matchStatus.default("scheduled"),
  homeScore: external_exports.number().default(0),
  awayScore: external_exports.number().default(0),
  games: external_exports.array(gameResultSchema).optional(),
  homeSubmission: external_exports.any().optional(),
  awaySubmission: external_exports.any().optional(),
  confirmedBy: external_exports.string().optional(),
  submittedBy: external_exports.string().optional()
});

// src/schemas/match.schema.ts
var submitResultSchema = external_exports.object({
  homeScore: external_exports.number().min(0),
  awayScore: external_exports.number().min(0),
  games: external_exports.array(gameResultSchema).optional()
});
var confirmResultSchema = external_exports.object({
  homeScore: external_exports.number().min(0),
  awayScore: external_exports.number().min(0),
  games: external_exports.array(gameResultSchema).optional()
});
...
// src/db/schema.ts
var countries = sqliteTable("countries", {
...
var users = sqliteTable("users", {
  ...
  // player | organizer | admin | super_admin
  name: text("name"),
...
var organizers = sqliteTable("organizers", {
  id: text("id").primaryKey(),
...
var teams = sqliteTable("teams", {
  ...
  members: text("members", { mode: "json" }).$type().notNull(),
  createdAt: integer("created_at").notNull(),
  updatedAt: integer("updated_at").notNull()
});
var teamMembers = sqliteTable("team_members", {
  ...
  roster: text("roster", { mode: "json" }).$type(),
  homeVenue: text("home_venue"),
  createdAt: integer("created_at").notNull(),
  updatedAt: integer("updated_at").notNull()
});
var competitions = sqliteTable("competitions", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  ...
  gameStructure: text("game_structure", { mode: "json" }).$type(),
  scheduleConfig: text("schedule_config", { mode: "json" }).$type(),
  createdAt: integer("created_at").notNull(),
  updatedAt: integer("updated_at").notNull()
});
var teamInvitations = sqliteTable("team_invitations", {
  id: text("id").primaryKey(),
  teamId: text("team_id").notNull(),
  teamName: text("team_name").notNull(),
...
var matches = sqliteTable("matches", {
  ...
  games: text("games", { mode: "json" }).$type(),
...
async function signJwt(userId, secret) {
  ...
  return new SignJWT({ sub: userId }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("30d").sign(key);
}
__name(signJwt, "signJwt");
...
async function register(services, jwtSecret, data) {
  const { db } = services;
  ...
  const pendingInvitations = await db.select().from(teamInvitations).where(and(eq(teamInvitations.invitedEmail, data.email), eq(teamInvitations.status, "pending"))).all();
  for (const invitation of pendingInvitations) {
    await db.update(teamInvitations).set({ invitedUserId: user.id, updatedAt: Date.now() }).where(eq(teamInvitations.id, invitation.id));
...
async function login(services, jwtSecret, data) {
  const { db } = services;
...
__name(login, "login");
async function getMe(services, userId) {
  const { db } = services;
...
auth.post("/register", zValidator("json", registerSchema), async (c) => {
  ...
  const result = await register(services, c.env.JWT_SECRET, data);
  return c.json(result, 201);
});
auth.post("/login", zValidator("json", loginSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const result = await login(services, c.env.JWT_SECRET, data);
  return c.json(result);
});
auth.get("/me", authMiddleware, async (c) => {
  const services = createServices(c);
  const user = await getMe(services, c.get("userId"));
  return c.json(user);
});
var auth_routes_default = auth;

// src/services/user.service.ts
async function getUser(services, id) {
  const { db } = services;
...
async function updateUser(services, userId, data) {
  const { db } = services;
  const user = await db.select().from(users).where(eq(users.id, userId)).get();
  if (!user)
    throw AppError.notFound("User not found");
  const updated = await db.update(users).set({ ...data, updatedAt: Date.now() }).where(eq(users.id, userId)).returning().get();
  const { passwordHash, ...profile } = updated;
  return profile;
}
__name(updateUser, "updateUser");
async function getUserTeams(services, userId) {
  const { db } = services;
  const allTeams = await db.select().from(teams).all();
  return allTeams.filter((t) => t.members.some((m) => m.playerId === userId));
}
__name(getUserTeams, "getUserTeams");
async function getUserStats(services, userId) {
  ...
  for (const match2 of userMatches) {
    const isHome = teamIds.includes(match2.homeTeamId);
    if (match2.homeScore > match2.awayScore) {
      isHome ? wins++ : losses++;
    } else if (match2.homeScore < match2.awayScore) {
      isHome ? losses++ : wins++;
    } else {
      draws++;
    }
...
userRoutes.get("/:id", async (c) => {
  const services = createServices(c);
  const user = await getUser(services, c.req.param("id"));
  return c.json(user);
});
userRoutes.put("/:id", zValidator("json", updateProfileSchema), async (c) => {
  ...
  const data = c.req.valid("json");
  const services = createServices(c);
  const user = await updateUser(services, userId, data);
  return c.json(user);
});
userRoutes.get("/:id/teams", async (c) => {
  const services = createServices(c);
  const teams3 = await getUserTeams(services, c.req.param("id"));
  return c.json(teams3);
});
userRoutes.get("/:id/stats", async (c) => {
  const services = createServices(c);
  const stats = await getUserStats(services, c.req.param("id"));
  return c.json(stats);
});
userRoutes.post("/:id/avatar", async (c) => {
  ...
  const services = createServices(c);
  const user = await updateUser(services, userId, { avatarUrl });
  return c.json(user);
});
var user_routes_default = userRoutes;

// src/services/team.service.ts
async function enrichMembers(db, members) {
  if (members.length === 0)
    return [];
  ...
  return members.map((m) => {
    const user = userMap.get(m.playerId);
    return { ...m, name: user?.name ?? "Unknown", nickname: user?.nickname ?? null, avatarUrl: user?.avatarUrl ?? null };
  });
}
__name(enrichMembers, "enrichMembers");
async function createTeam(services, captainId, data) {
  const { db } = services;
  const now = Date.now();
  return db.insert(teams).values({
    ...
    members: [{ playerId: captainId, role: "captain", joinedAt: (/* @__PURE__ */ new Date()).toISOString() }],
    createdAt: now,
    updatedAt: now
  }).returning().get();
}
__name(createTeam, "createTeam");
async function getTeam(services, id) {
  const { db } = services;
...
async function listTeams(services, limit = 20, offset = 0) {
  const { db } = services;
  const data = await db.select().from(teams).orderBy(desc(teams.createdAt)).limit(limit).offset(offset).all();
  const totalResult = await db.select({ value: sql`count(*)` }).from(teams).get();
  const total = totalResult?.value ?? 0;
  return { data, total, hasMore: offset + limit < total };
}
__name(listTeams, "listTeams");
async function updateTeam(services, teamId, userId, data) {
  const { db } = services;
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (!team)
    throw AppError.notFound("Team not found");
  if (team.captainId !== userId)
    throw AppError.forbidden("Only captain can update team");
  return db.update(teams).set({ ...data, updatedAt: Date.now() }).where(eq(teams.id, teamId)).returning().get();
}
__name(updateTeam, "updateTeam");
async function invitePlayerByEmail(services, teamId, captainId, email) {
  const { db } = services;
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (!team)
    throw AppError.notFound("Team not found");
  if (team.captainId !== captainId)
    throw AppError.forbidden("Only captain can invite players");
  ...
  if (user) {
    ...
    const existing2 = await db.select().from(teamInvitations).where(and(
  ...
  const existing = await db.select().from(teamInvitations).where(and(
    eq(teamInvitations.teamId, teamId),
...
__name(invitePlayerByEmail, "invitePlayerByEmail");
async function respondToInvitation(services, invitationId, userId, accept) {
  const { db } = services;
  ...
  if (invitation.status !== "pending")
    throw AppError.badRequest("Invitation already responded to");
  const newStatus = accept ? "accepted" : "rejected";
  await db.update(teamInvitations).set({ status: newStatus, updatedAt: Date.now() }).where(eq(teamInvitations.id, invitationId));
  if (accept) {
    ...
    if (team) {
      ...
      await db.update(teams).set({ members: updatedMembers, updatedAt: Date.now() }).where(eq(teams.id, invitation.teamId));
    }
  }
...
var teams2 = new Hono2();
teams2.use("*", authMiddleware);
teams2.post("/", zValidator("json", createTeamSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const team = await createTeam(services, c.get("userId"), data);
...
teams2.get("/", async (c) => {
  ...
  const allTeams = await listTeams(services, limit, offset);
  return c.json(allTeams);
});
teams2.get("/:id", async (c) => {
  const services = createServices(c);
  const team = await getTeam(services, c.req.param("id"));
  return c.json(team);
});
teams2.put("/:id", zValidator("json", updateTeamSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const team = await updateTeam(services, c.req.param("id"), c.get("userId"), data);
  return c.json(team);
});
teams2.post("/:id/invite", zValidator("json", invitePlayerSchema), async (c) => {
  const { email } = c.req.valid("json");
  const services = createServices(c);
  const invitation = await invitePlayerByEmail(services, c.req.param("id"), c.get("userId"), email);
  return c.json(invitation, 201);
});
var team_routes_default = teams2;
...
async function createCompetition(services, organizerId, data) {
  const { db } = services;
  const now = Date.now();
...
async function getCompetition(services, id) {
  const { db } = services;
...
__name(getCompetition, "getCompetition");
async function listCompetitions(services, limit = 20, offset = 0) {
  const { db } = services;
  const data = await db.select().from(competitions).orderBy(desc(competitions.createdAt)).limit(limit).offset(offset).all();
  const totalResult = await db.select({ value: sql`count(*)` }).from(competitions).get();
  const total = totalResult?.value ?? 0;
  return { data, total, hasMore: offset + limit < total };
}
__name(listCompetitions, "listCompetitions");
async function updateCompetition(services, id, userId, data) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, id)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.organizerId !== userId)
    throw AppError.forbidden("Only organizer can update");
  if (comp.status !== "draft")
    throw AppError.badRequest("Can only update draft competitions");
  const { id: _, createdAt, updatedAt, organizerId, status, ...updateFields } = data;
  return db.update(competitions).set({ ...updateFields, updatedAt: Date.now() }).where(eq(competitions.id, id)).returning().get();
}
__name(updateCompetition, "updateCompetition");
...
async function publishCompetition(services, id, userId) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, id)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.organizerId !== userId)
    throw AppError.forbidden();
  validateTransition(comp.status, "upcoming");
  return db.update(competitions).set({ status: "upcoming", updatedAt: Date.now() }).where(eq(competitions.id, id)).returning().get();
}
__name(publishCompetition, "publishCompetition");
async function closeApplications(services, id, userId) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, id)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.organizerId !== userId)
    throw AppError.forbidden();
  validateTransition(comp.status, "active");
  const pendingApps = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, id), eq(teamMembers.status, "pending"))).all();
  for (const app2 of pendingApps) {
    await db.update(teamMembers).set({ status: "rejected", updatedAt: Date.now() }).where(eq(teamMembers.id, app2.id));
  }
  return db.update(competitions).set({ status: "active", updatedAt: Date.now() }).where(eq(competitions.id, id)).returning().get();
}
__name(closeApplications, "closeApplications");

// src/services/scheduling.service.ts
function bergerRoundRobin(teams3) {
  const n = teams3.length;
  const usesBye = n % 2 !== 0;
...
__name(bergerRoundRobin, "bergerRoundRobin");
function assignDates(fixtures, config, startDate) {
  ...
  if (config.schedulingType === "weekly_rounds" && config.weekdays?.length) {
    ...
    for (const fixture of fixtures) {
      ...
      result.push({
        ...
        venue: config.venueType === "central" ? config.centralVenue : void 0
      });
...
__name(assignDates, "assignDates");
async function generateMatches(services, competitionId, userId) {
  const { db } = services;
  ...
  if (comp.status !== "active")
    throw AppError.badRequest("Competition must be active to generate matches");
  const existingMatches = await db.select().from(matches).where(eq(matches.competitionId, competitionId)).all();
  if (existingMatches.length > 0) {
    throw AppError.conflict("Matches already generated for this competition");
  }
  const accepted = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.status, "accepted"))).all();
  ...
  const scheduleConfig = comp.scheduleConfig;
...
__name(generateMatches, "generateMatches");

// src/routes/competition.routes.ts
var competitions2 = new Hono2();
competitions2.use("*", authMiddleware);
competitions2.post("/", requireRole("organizer", "admin"), zValidator("json", createCompetitionSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const comp = await createCompetition(services, c.get("userId"), data);
  return c.json(comp, 201);
});
competitions2.get("/", async (c) => {
  const services = createServices(c);
  const limit = Math.min(parseInt(c.req.query("limit") ?? "20"), 100);
  const offset = parseInt(c.req.query("offset") ?? "0");
  const comps = await listCompetitions(services, limit, offset);
  return c.json(comps);
});
competitions2.get("/:id", async (c) => {
  const services = createServices(c);
  const comp = await getCompetition(services, c.req.param("id"));
  return c.json(comp);
});
competitions2.put("/:id", zValidator("json", updateCompetitionSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const comp = await updateCompetition(services, c.req.param("id"), c.get("userId"), data);
  return c.json(comp);
});
competitions2.post("/:id/publish", async (c) => {
  const services = createServices(c);
  const comp = await publishCompetition(services, c.req.param("id"), c.get("userId"));
  return c.json(comp);
});
competitions2.post("/:id/close-applications", async (c) => {
  const services = createServices(c);
  const comp = await closeApplications(services, c.req.param("id"), c.get("userId"));
  return c.json(comp);
});
competitions2.post("/:id/generate-matches", async (c) => {
  const services = createServices(c);
  const matches3 = await generateMatches(services, c.req.param("id"), c.get("userId"));
  return c.json(matches3, 201);
});
var competition_routes_default = competitions2;

// src/services/participation.service.ts
async function applyToCompetition(services, competitionId, teamId, userId) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, competitionId)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.status !== "upcoming")
    throw AppError.badRequest("Competition is not accepting applications");
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (!team)
    throw AppError.notFound("Team not found");
  if (team.captainId !== userId)
    throw AppError.forbidden("Only team captain can apply");
  ...
  const existing = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.teamId, teamId))).all();
  if (existing.length > 0) {
  ...
  return db.insert(teamMembers).values({
    ...
    roster: team.members.map((m) => ({
      playerId: m.playerId,
      name: m.playerId
      // Will be resolved at display time
    })),
    createdAt: now,
    updatedAt: now
  }).returning().get();
}
__name(applyToCompetition, "applyToCompetition");
async function handleApplication(services, competitionId, teamId, userId, action) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, competitionId)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.organizerId !== userId)
    throw AppError.forbidden();
  const apps = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.teamId, teamId))).all();
  if (apps.length === 0)
    throw AppError.notFound("Application not found");
  const app2 = apps[0];
  if (app2.status !== "pending")
    throw AppError.badRequest("Application already handled");
  const newStatus = action === "accept" ? "accepted" : "rejected";
  const updated = await db.update(teamMembers).set({ status: newStatus, updatedAt: Date.now() }).where(eq(teamMembers.id, app2.id)).returning().get();
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (team) {
    const now = Date.now();
    await db.insert(notifications).values({
...
__name(handleApplication, "handleApplication");
async function getCompetitionParticipations(services, competitionId) {
  const { db } = services;
  return db.select().from(teamMembers).where(eq(teamMembers.competitionId, competitionId)).all();
}
__name(getCompetitionParticipations, "getCompetitionParticipations");
async function respondToCompetitionInvite(services, competitionId, teamId, userId, accept) {
  const { db } = services;
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (!team)
    throw AppError.notFound("Team not found");
  if (team.captainId !== userId)
    throw AppError.forbidden("Only team captain can respond");
  const apps = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.teamId, teamId))).all();
  if (apps.length === 0)
    throw AppError.notFound("Invitation not found");
  const app2 = apps[0];
  if (app2.status !== "invited")
    throw AppError.badRequest("No pending invitation");
...
__name(respondToCompetitionInvite, "respondToCompetitionInvite");
async function getTeamCompetitionInvitations(services, userId) {
  const { db } = services;
  ...
  for (const team of userTeams) {
    const teamInvites = await db.select().from(teamMembers).where(and(eq(teamMembers.teamId, team.id), eq(teamMembers.status, "invited"))).all();
    for (const invite of teamInvites) {
      const comp = await db.select().from(competitions).where(eq(competitions.id, invite.competitionId)).get();
      invitations2.push({ ...invite, competitionName: comp?.name ?? "Unknown" });
    }
  }
  return invitations2;
}
__name(getTeamCompetitionInvitations, "getTeamCompetitionInvitations");
async function inviteTeamToCompetition(services, competitionId, teamId, organizerUserId) {
  const { db } = services;
  const comp = await db.select().from(competitions).where(eq(competitions.id, competitionId)).get();
  if (!comp)
    throw AppError.notFound("Competition not found");
  if (comp.organizerId !== organizerUserId)
    throw AppError.forbidden("Only competition organizer can invite teams");
  if (comp.status !== "upcoming")
    throw AppError.badRequest("Competition is not accepting teams");
  const team = await db.select().from(teams).where(eq(teams.id, teamId)).get();
  if (!team)
    throw AppError.notFound("Team not found");
  const existing = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.teamId, teamId))).all();
...
__name(inviteTeamToCompetition, "inviteTeamToCompetition");

// src/routes/participation.routes.ts
var participations = new Hono2();
participations.use("*", authMiddleware);
participations.post(
  "/competitions/:competitionId/apply",
  zValidator("json", external_exports.object({ teamId: external_exports.string() })),
  async (c) => {
    const { teamId } = c.req.valid("json");
    const services = createServices(c);
    const app2 = await applyToCompetition(
      services,
...
participations.put(
  ...
  async (c) => {
    ...
    const services = createServices(c);
    const app2 = await handleApplication(
      services,
      c.req.param("competitionId"),
      c.req.param("teamId"),
      c.get("userId"),
...
participations.post(
  ...
  async (c) => {
    const { teamId } = c.req.valid("json");
    const services = createServices(c);
    const result = await inviteTeamToCompetition(
      services,
      c.req.param("competitionId"),
      teamId,
      c.get("userId")
...
participations.post(
  ...
  zValidator("json", external_exports.object({ accept: external_exports.boolean() })),
  async (c) => {
    const { accept } = c.req.valid("json");
    const services = createServices(c);
    const result = await respondToCompetitionInvite(
      services,
      c.req.param("competitionId"),
      c.req.param("teamId"),
      c.get("userId"),
...
participations.get("/competition-invitations", async (c) => {
  const services = createServices(c);
  const invitations2 = await getTeamCompetitionInvitations(services, c.get("userId"));
  return c.json(invitations2);
});
participations.get("/competitions/:competitionId/participations", async (c) => {
  const services = createServices(c);
  const apps = await getCompetitionParticipations(services, c.req.param("competitionId"));
  return c.json(apps);
});
var participation_routes_default = participations;

// src/services/match.service.ts
function parseSubmission(raw2) {
  if (!raw2)
    return null;
  try {
...
async function getCompetitionMatches(services, competitionId) {
  const { db } = services;
  return db.select().from(matches).where(eq(matches.competitionId, competitionId)).all();
}
__name(getCompetitionMatches, "getCompetitionMatches");
async function getMatch(services, id) {
  const { db } = services;
  const match2 = await db.select().from(matches).where(eq(matches.id, id)).get();
  if (!match2)
    throw AppError.notFound("Match not found");
  return match2;
}
__name(getMatch, "getMatch");
async function submitResult(services, matchId, userId, data) {
  const { db } = services;
  const match2 = await db.select().from(matches).where(eq(matches.id, matchId)).get();
  if (!match2)
    throw AppError.notFound("Match not found");
  if (match2.status === "completed") {
    throw AppError.badRequest("Match result already confirmed");
  }
  const comp = await db.select().from(competitions).where(eq(competitions.id, match2.competitionId)).get();
  const homeTeam = await db.select().from(teams).where(eq(teams.id, match2.homeTeamId)).get();
  const awayTeam = await db.select().from(teams).where(eq(teams.id, match2.awayTeamId)).get();
  ...
  if (isOrganizer) {
    const updated = await db.update(matches).set({
      ...
      submittedBy: userId,
      updatedAt: now
    }).where(eq(matches.id, matchId)).returning().get();
    const metadata = JSON.stringify({
      matchId,
      homeTeamName: match2.homeTeamName,
      awayTeamName: match2.awayTeamName,
      homeScore: data.homeScore,
      awayScore: data.awayScore
    });
    const captainIds = [homeTeam?.captainId, awayTeam?.captainId].filter(Boolean);
    for (const captainId of captainIds) {
      await db.insert(notifications).values({
        ...
        message: `${match2.homeTeamName} ${data.homeScore}\u2013${data.awayScore} ${match2.awayTeamName} has been confirmed`,
        read: false,
        referenceId: matchId,
        referenceType: "match",
        metadata,
  ...
  const otherSubmission = parseSubmission(isHome ? match2.awaySubmission : match2.homeSubmission);
  ...
  if (otherSubmission) {
    if (otherSubmission.homeScore === data.homeScore && otherSubmission.awayScore === data.awayScore) {
      ...
      if (comp) {
        ...
        await db.insert(notifications).values({
          ...
          message: `${match2.homeTeamName} vs ${match2.awayTeamName}: captains submitted different scores`,
          read: false,
          referenceId: matchId,
          referenceType: "match",
          metadata,
    ...
    if (otherCaptainId) {
      await db.insert(notifications).values({
        ...
        message: `${submitterName} submitted ${match2.homeTeamName} ${data.homeScore}\u2013${data.awayScore} ${match2.awayTeamName}`,
        read: false,
        referenceId: matchId,
        referenceType: "match",
        metadata,
...
async function confirmResult(services, matchId, userId, data) {
  const { db } = services;
  const match2 = await db.select().from(matches).where(eq(matches.id, matchId)).get();
  if (!match2)
    throw AppError.notFound("Match not found");
  ...
  for (const captainId of captainIds) {
    await db.insert(notifications).values({
      ...
      type: "score_confirmed",
      title: "Score Confirmed",
      message: `${match2.homeTeamName} ${data.homeScore}\u2013${data.awayScore} ${match2.awayTeamName} has been confirmed by the organizer`,
      read: false,
      referenceId: matchId,
      referenceType: "match",
      metadata,
...
__name(confirmResult, "confirmResult");

// src/routes/match.routes.ts
var matches2 = new Hono2();
matches2.use("*", authMiddleware);
matches2.get("/competitions/:competitionId/matches", async (c) => {
  const services = createServices(c);
  const results = await getCompetitionMatches(services, c.req.param("competitionId"));
  return c.json(results);
});
matches2.get("/:id", async (c) => {
  const services = createServices(c);
  const match2 = await getMatch(services, c.req.param("id"));
  return c.json(match2);
});
matches2.post("/:id/result", zValidator("json", submitResultSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const match2 = await submitResult(services, c.req.param("id"), c.get("userId"), data);
  return c.json(match2);
});
matches2.post("/:id/confirm", zValidator("json", confirmResultSchema), async (c) => {
  const data = c.req.valid("json");
  const services = createServices(c);
  const match2 = await confirmResult(services, c.req.param("id"), c.get("userId"), data);
  return c.json(match2);
});
var match_routes_default = matches2;

// src/services/standings.service.ts
async function getStandings(services, competitionId) {
  const { db } = services;
  const participations2 = await db.select().from(teamMembers).where(and(eq(teamMembers.competitionId, competitionId), eq(teamMembers.status, "accepted"))).all();
  const allMatches = await db.select().from(matches).where(eq(matches.competitionId, competitionId)).all();
  const completedMatches = allMatches.filter((m) => m.status === "completed");
  const standingsMap = /* @__PURE__ */ new Map();
  for (const p of participations2) {
  ...
  for (const match2 of completedMatches) {
    const home = standingsMap.get(match2.homeTeamId);
    const away = standingsMap.get(match2.awayTeamId);
    if (home) {
      home.played++;
      home.gamesWon += match2.homeScore;
      home.gamesLost += match2.awayScore;
      if (match2.homeScore > match2.awayScore) {
        home.won++;
        home.points += 3;
      } else if (match2.homeScore === match2.awayScore) {
    ...
    if (away) {
      away.played++;
      away.gamesWon += match2.awayScore;
      away.gamesLost += match2.homeScore;
      if (match2.awayScore > match2.homeScore) {
        away.won++;
        away.points += 3;
      } else if (match2.homeScore === match2.awayScore) {
        away.drawn++;
...
var standings = new Hono2();
standings.get("/competitions/:competitionId/standings", authMiddleware, async (c) => {
  const services = createServices(c);
  const results = await getStandings(services, c.req.param("competitionId"));
  return c.json(results);
});
var standings_routes_default = standings;

// src/services/notification.service.ts
async function getPlayerNotifications(services, userId, limit = 20, offset = 0) {
  const { db } = services;
  const data = await db.select().from(notifications).where(eq(notifications.userId, userId)).orderBy(desc(notifications.createdAt)).limit(limit).offset(offset).all();
  const totalResult = await db.select({ value: sql`count(*)` }).from(notifications).where(eq(notifications.userId, userId)).get();
  const total = totalResult?.value ?? 0;
  return { data, total, hasMore: offset + limit < total };
}
__name(getPlayerNotifications, "getPlayerNotifications");
async function markNotificationRead(services, notificationId, userId) {
  const { db } = services;
  ...
  if (notification.userId !== userId)
    throw AppError.forbidden();
  return db.update(notifications).set({ read: true, updatedAt: Date.now() }).where(eq(notifications.id, notificationId)).returning().get();
}
__name(markNotificationRead, "markNotificationRead");

// src/routes/notification.routes.ts
var notifications2 = new Hono2();
notifications2.use("*", authMiddleware);
notifications2.get("/", async (c) => {
  const services = createServices(c);
  const limit = Math.min(parseInt(c.req.query("limit") ?? "20"), 100);
  const offset = parseInt(c.req.query("offset") ?? "0");
  const results = await getPlayerNotifications(services, c.get("userId"), limit, offset);
  return c.json(results);
});
notifications2.put("/:id/read", async (c) => {
  const services = createServices(c);
...
var notification_routes_default = notifications2;

// src/routes/invitation.routes.ts
var invitations = new Hono2();
invitations.use("*", authMiddleware);
invitations.get("/pending", async (c) => {
  const { db } = createServices(c);
  const pending = await db.select().from(teamInvitations).where(and(
...
var invitation_routes_default = invitations;

// src/services/city.service.ts
async function searchCities(db, countryCode, query) {
  ...
  }).from(cities).innerJoin(countries, eq(cities.countryCode, countries.code)).where(and(...conditions)).orderBy(cities.name).limit(50).all();
  return results;
}
__name(searchCities, "searchCities");
async function listCountries(db) {
  return db.select().from(countries).orderBy(countries.name).all();
}
__name(listCountries, "listCountries");

// src/routes/city.routes.ts
var cityRoutes = new Hono2();
cityRoutes.get("/", async (c) => {
  const db = getDb(c.env.DB);
  const country = c.req.query("country");
  const q = c.req.query("q");
  const results = await searchCities(db, country, q);
  return c.json(results);
});
var countryRoutes = new Hono2();
countryRoutes.get("/", async (c) => {
  const db = getDb(c.env.DB);
  const results = await listCountries(db);
  return c.json(results);
});
...
seed.post("/seed", async (c) => {
  ...
  for (const m of matchDefs) {
    const match2 = await db.insert(matches).values({
      ...
      homeScore: m.hs,
      awayScore: m.as,
      games: "games" in m ? m.games : void 0,
      submittedBy: m.status === "completed" ? toan.id : void 0,
      createdAt: now,
      updatedAt: now
    }).returning().get();
    createdMatches.push(match2);
  ...
  const notificationDefs = [
    ...
    },
    {
      ...
      read: true,
      referenceId: createdMatches[0].id,
      referenceType: "match"
    },
    {
      userId: toan.id,
      ...
      read: false,
      referenceId: createdMatches[4].id,
      referenceType: "match"
    },
    {
      userId: toan.id,
    ...
    },
    {
      ...
      read: false,
      referenceId: createdMatches[1].id,
      referenceType: "match"
    },
...
// src/routes/index.ts
function mountRoutes(app2) {
  app2.route("/api", seed_default);
  app2.route("/api/auth", auth_routes_default);
  app2.route("/api/users", user_routes_default);
  app2.route("/api/teams", team_routes_default);
  app2.route("/api/competitions", competition_routes_default);
  app2.route("/api", participation_routes_default);
  app2.route("/api/matches", match_routes_default);
  app2.route("/api", standings_routes_default);
  app2.route("/api/notifications", notification_routes_default);
  app2.route("/api/team-invitations", invitation_routes_default);
  app2.route("/api/cities", cityRoutes);
  app2.route("/api/countries", countryRoutes);
}
__name(mountRoutes, "mountRoutes");
...
// .wrangler/tmp/bundle-YG7cK3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YG7cK3/middleware-loader.entry.ts
